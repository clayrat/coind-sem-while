...
- inv hexec.
  ...
  * clear st0 hu H3. move: h1 => [tr1[_ h1]].
     ...
     - inversion h0. clear H3 H1 H0. move: H2 => [st1 [h1 h2]].
       ...
       apply follows_nil => //. by apply: (IHsemax2 _ _ H h1).
     - inv h0. by apply: (follows_delay e (CIH _ _ H1 H)).
...
- clear H0. destruct p as [p hp]. simpl in IHsemax. simpl.
  ...
  * clear st0 tr0 hexec hu h0.
    ...
    * cofix CIH=> tr0 tr1 h1. inv h1.
      + inv H0.
        - apply/follows_delay/follows_nil=>//.
          exists st. by split; [apply: H5| apply: bisim_reflexive].
        inv H4. inv H6. inv H7.
        by apply: (follows_delay st (CIH _ _ H6)).
      by apply: (follows_delay e (CIH _ _ H0)).
    move => st0 tr0 h0. inv h0. apply/follows_delay/follows_nil=>//.
    exists st0. split; [apply: H4|apply: bisim_reflexive].
    by apply: (h1 _ _ H6).
  inversion hexec; subst.
  - exists (Tcons st0 (Tnil st0)). split.
    * exists st0. by split; [apply: hu| apply: bisim_reflexive].
    apply/follows_delay/follows_nil => //.
    exists (Tnil st0). split; first by apply: iter_stop.
    apply follows_nil => //. exists st0. by split; [apply: H4| apply: bisim_reflexive].
  - inversion H3; subst. inversion H7; subst. inversion H6; subst.
    ...
    * by apply: (h0 _ (H _ hu) _ hexec).
    have h2 := h1 _ _ hexec. inv h2. by apply: H1.
