...
by exact: hsemax_conseq h0 (@assertS_imp_refl v) h1.
Qed.
...
by apply: (hsemax_conseq (@assertS_imp_refl u) h0 h1).
Qed.
...
- move => w.
  ...
  * move => st [{}h0 h1]. do!split => //. clear h1.
    ...
    * apply: iter_last=>/=. exists (Tnil (hd tr')). split.
      - exists (hd tr'). by split => //; apply: bisim_nil.
      apply follows_nil => //.
...
- move => v.
  ...
  * move => st0 [x h0]. move hp: (p x) => [q hq].
    ...
    exists tr1. split => // {h0}. move: tr1 tr0 h1.
    cofix CIH=> tr0 tr1 h0. inv h0.
...
apply: (hsemax_conseq _ _ hhsemax) => {hhsemax}st0.
 * move => hU. by split.
 * move => h0. destruct P as [P hP]. destruct h0 as [tr0 [h0 h1]].
...
- have h0 := semax_skip u.
  ...
  * move => tr0 [st0 [h1 h2]]. inv h2. exists (Tnil st0). split => //.
    apply follows_nil =>//. exists st0. by split=>//; apply: bisim_nil.
...
- have h0: u ->> u by [].
  ...
  * move => tr0 [tr1 [[st0 [h0 h2]] {}h1]].
    ...
    cofix CIH0=> tr0 [tr1 [h0 h2]] h1.
    inv h0.
    + inv h2. move: H2 => [st0 [h0 h2]]. inv h2. simpl in h1.
      apply follows_nil => //. exists st0. split => //. by apply: bisim_reflexive.
    clear h1. move: tr tr1 tr0 H0 H1 h2.
    cofix CIH1=> tr0 tr1 tr2 [tr3 [h0 h3]] h1 h2. inv h3.
    + clear h0. move: H1 => [st0 [h0 h3]]. inv h3. inv H2. inv h1.
      inv H3. inv h2. rewrite (follows_hd H4) in h0.
      apply: (follows_delay (hd tr')); apply: (CIH0 _ _ h0).
      by exists tr'.
    inv h1. inv h2.
    clear h0 CIH0. apply follows_delay.
    apply: (CIH1 _ _ _ _ H4 H5). by exists tr.
