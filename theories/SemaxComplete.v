...
- move => [st [h1 h2]]. subst. inv h2. simpl. by apply h1.
- move => [st0 [h1 h2]]. inv h2. inv H1. simpl. by apply h1.
- move => [tr0 [h1 h2]].
   have h3 := IHs1 _ _ h1.
   rewrite (follows_hd h2) in h3; by apply h3.
- move => [tr0 [[st1 [h1 h3]] h2]]. inv h3.
  inv H1. inv h2. by simpl.
- move => [tr0 [[st1 [h1 h3]] h2]]. inv h3. inv H1.
  inv h2. by simpl.
Qed.
...
- apply: loopinv_here. apply: (h1 _ H).
...
- move: h2 => [st1 [h2 h3]]. inv h3. exists st1. split.
  apply: (h1 _ h2). reflexivity.
- move: h2 => [st0 [h3 h2]]. inv h2. inv H1. exists st0. split.
  apply: (h1 _ h3). reflexivity.
...
- move: h2 => [tr1 [h2 h3]]. move: h2 => [st0 [h2 h4]]. inv h4.
  inv h3. inv H1. inv H3. exists (Tcons (hd tr') (Tnil (hd tr'))). split.
  * exists (hd tr'). split.
    * have := h1 _ h2; apply.
    * reflexivity.
...
- move => u tr [st [h1 h2]]. inv h2. simpl. by apply: exec_skip.
- move => u tr [st [h1 h2]]. inv h2. inv H1. simpl. apply: exec_assign.
- move => u tr [tr0 [h1 h2]].
...
- move => u tr h1. move: h1 => [tr0 [h1 h2]]. move: h1 => [st0 [h1 h3]].
  inv h3. inv h2. inv H1. inv H3. simpl. case: H1 => h4.
  - move: (sp_hd h4) => [h5 h6].
...
- move => u tr0 [tr1 [[st0 [h1 h3]] h2]].
  inv h3. inv H1. inv h2. inv H2. simpl. move => {h1}.
  ...
  * cofix COINDHYP0. move => tr0 tr1 tr2 tr3 h0 h2 h3.
    inv h0. move: H0 => [st0 [_ h1]]. inv h1. inv H1. inv h2.
    inv H2. inv h3. simpl. rewrite (follows_hd H3). apply execseq_nil.
  ...
  * move => tr0 tr1 h0 h1.  inv h0.
    - inv h1. move: H1 => [st0 [h0 h1]]. inv h1. simpl.
      inv h0. have :=  exec_while_false _ H0. apply.
    - move: H => [tr2 [h0 h2]].
...
- move => [tr2 [h1 h2]] h3. exists tr2. split.
  ...
  * move => {h1}. move: tr0 tr1 tr2 h3 h2. cofix COINDHYP.
     move => tr0 tr1 tr2 h3 h2. inv h2.
    - apply: follows_nil.
      * symmetry. have := bisim_hd h3. apply.
      *  have := IHs2 _ _ _ H0; apply. by apply: h3.
    - inv h3. have := follows_delay st (COINDHYP _ _ _ H3 H); apply.
- move => [tr2 [[st0 [h1 h4]] h2]] h3. inv h4.
  inv H1. inv h2. inv h3. inv H2. have h2 := bisim_hd H3.
...
- move => [tr2 [h0 h2]] h1. exists tr2.
  split; first by apply h0. move: h0 => [st0 [h0 h3]]. inv h3. inv H1.
  inv h2. inv H2. inv h1. apply follows_delay. have h1 := bisim_hd H3.
  symmetry in h1. apply: (follows_nil h1) => {h1 h0}.
  apply: (append_setoid _ H1 H3). by apply singleton_setoid.
Qed.
...
move: (follows_singleton h1) => {}h1.
by apply: (sp_setoid h0 h1).
Qed.
...
- have h1 := IHs1 (u andS eval_true e) => {IHs1}.
  ...
  * clear h0. move => tr0 h0. simpl. simpl in h0. move: h0 => [tr1 [h0 h1]].
     exists tr1. split; first by apply h0. move: h0 => [st0 [h0 h2]]. inv h2. inv H1.
     inv h1. inv H2. clear h0. apply follows_delay. apply follows_nil => //.
...
- have h0 := IHs (loopinv e (sp s) u andS eval_true e) => {IHs}.
  ...
  * clear h0 h1. move => tr0 h0. simpl in h0. simpl. apply loopinv_adequate.
  ...
  << loopinv e (sp s) u>>) *** [| eval_false e|]) =>>
   (Sp (Swhile e s) u).
  * move => tr0 {}h0 /=. apply h0.
  apply: (semax_conseq_R h1 h0).
Qed.
