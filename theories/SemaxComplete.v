...
- move => h0.
  have h1 := h0 st0 (fin_nil st0) => {h0}.
  apply follows_nil=>//. exists st0.
  by split=>//; apply: bisim_reflexive.
- move => tr0 h0.
  have h1: forall st, fin tr0 st -> u st.
  * move => st1 h1. by apply: (h0 st1 (fin_delay st0 h1)).
  by apply: (follows_delay st0 (CIH _ h1)).
Qed.
...
- by apply: loopinv_next; [exact: IHloopinv| |exact: H1|exact: H3 | | ].
Qed.
...
- move: h2 => [st1 [h2 h3]]. inv h3. exists st1.
  by split; [apply: (h1 _ h2)| apply: bisim_reflexive].
- move: h2 => [st0 [h3 h2]]. inv h2. inv H1. exists st0.
  by split; [apply: (h1 _ h3)| apply: bisim_reflexive].
...
- move: h2 => [tr1 [[st0 [h2 h4]] h3]].
  inv h4. inv h3. inv H1. inv H3. exists (Tcons (hd tr') (Tnil (hd tr'))). split.
  * exists (hd tr'). by split; [apply: (h1 _ h2) | apply: bisim_reflexive].
...
- move => u tr [tr0 [h1 h2]].
  ...
  - by apply/execseq_nil/(IHs2 _ _ H0).
  - by apply/execseq_cons/(CIH _  _ H).
- move => u tr [tr0 [[st0 [h1 h3]] h2]].
  ...
  - move: (sp_hd h4) => [h5 h6].
    ...
    by apply: (IHs1 _ _ h4).
  - move: (sp_hd h4) => [h5 h6].
    apply: (exec_ifthenelse_false _ h6).
    apply: execseq_cons. apply: execseq_nil.
    by apply: (IHs2 _ _ h4).
- move => u tr0 [tr1 [[st0 [h1 h3]] h2]].
  ...
  * cofix CIH0=> tr0 tr1 tr2 tr3 h0 h2 h3.
    ...
    inv h1. inv H1. inv h2. inv H2. inv h3. simpl.
...
- move => [tr2 [h1 h2]] h3. exists tr2. split.
  ...
  * move => {h1}. move: tr0 tr1 tr2 h3 h2.
    cofix CIH=> tr0 tr1 tr2 h3 h2. inv h2.
...
- move => [tr2 [[st0 [h1 h4]] h2]] h3. inv h4.
  ...
  have h2 := bisim_hd H3.
  exists (Tcons (hd tr') (Tnil (hd tr'))). split.
  * exists (hd tr'). by split; [apply h1 | reflexivity].
  apply/follows_delay/follows_nil; first by apply/esym.
  clear h1 h2. move: H1 => [h1 | h1].
...
- have h0: ([|u|]) =>> (Sp Sskip u).
  * move => tr0 [st0 [h0 h1]]. inv h1. exists st0.
    by split; [apply h0 | reflexivity].
  by apply: (semax_conseq_R h0 (semax_skip _)).
- have h0: (Updt u i e) =>> Sp (i <- e) u.
  * move => tr0 h0. move: h0 => [st0 [h0 h1]]. inv h1. inv H1.
     exists st0. by split=>//; apply: bisim_reflexive.
  by apply: (semax_conseq_R h0 (semax_assign _ _ _)).
- have h0 := IHs1 u => {IHs1}.
